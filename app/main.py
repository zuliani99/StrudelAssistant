        ...
        with st.chat_message("assistant"):
            try:
                ...
                with st.spinner("Retreiving docs and generating response..."):
                    # messages[:-1]: everything before the prompt just appended above,
                    # so the agent sees the whole conversation without seeing this
                    # turn's question twice.
                    history = [
                        {"role": m["role"], "content": m["content"]}
                        for m in st.session_state.messages[:-1]
                    ]
                    result: Dict[str, Any] = run_llm(prompt, chat_history=history)
